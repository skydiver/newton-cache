...
(async () => {
  ...
  const user1 = await cache.remember('user:123', 10, () => fetchUserFromAPI(123));
  console.log('   First call result:', user1);

  // Second call - cache hit, factory NOT run
  const user2 = await cache.remember('user:123', 10, () => fetchUserFromAPI(123));
  ...
  await getProducts(); // Cache hit - no DB query
  await getProducts(); // Cache hit - no DB query
  ...
  const config1 = await cache.rememberForever('app-config', loadAppConfig);
  console.log('   First call:', config1);

  const config2 = await cache.rememberForever('app-config', loadAppConfig);
  console.log('   Second call:', config2);

  console.log('   Total computations:', computationCount, '(should be 1)');
  console.log('   TTL:', await cache.ttl('app-config'), '(null = permanent)');
  console.log();
  ...
  class UserService {
    ...
    async getUserProfile(userId) {
      return await this.cache.remember(`user-profile:${userId}`, 300, () => {
        this.apiCalls++;
    ...
    async getUserPosts(userId) {
      return await this.cache.remember(`user-posts:${userId}`, 180, () => {
        this.apiCalls++;
  ...
  async function warmCache() {
    ...
    for (const { key, factory } of criticalData) {
      await cache.rememberForever(key, () => {
  ...
  async function getWeatherData(city) {
    ...
    if (data.condition !== 'Error') {
      return await cache.remember(cacheKey, 1800, () => {
  ...
  await getWeatherData('San Francisco');
  await getWeatherData('San Francisco'); // Cached
  ...
  async function getFilteredProducts(category, minPrice) {
    const cacheKey = `products:${category}:${minPrice}`;

    return await cache.remember(cacheKey, 120, () => {
  ...
  await getFilteredProducts('electronics', 50);
  await getFilteredProducts('electronics', 50); // Cached
  await getFilteredProducts('electronics', 100); // Different key, not cached
  console.log();
  ...
  console.log('   All keys:', finalKeys.length, 'keys');

  await cache.flush();
  console.log('   After flush:', await cache.count(), 'entries');
  console.log();
