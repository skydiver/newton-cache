...
cache.putMany(users, 3600); // Store all with 1 hour TTL
...
const retrieved = cache.getMany(userKeys);
...
const mixedResults = cache.getMany(mixedKeys);
...
cache.putMany(products, 7200); // 2 hours TTL
const elapsed = Date.now() - start;

console.log(`   Cached ${Object.keys(products).length} products in ${elapsed}ms`);
console.log(`   Total cache entries: ${cache.count()}`);
console.log();
...
const selectedProducts = cache.getMany(productKeys);
...
const removedUsers = cache.forgetMany(userOnlyKeys);
