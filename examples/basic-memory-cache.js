...
cache.put('user:1', { id: 1, name: 'Alice', role: 'admin' });
cache.put('user:2', { id: 2, name: 'Bob', role: 'user' });
...
cache.put('temp-token', 'secret123', 2);
console.log('   Stored temp-token with 2-second TTL');
console.log('   Value:', cache.get('temp-token'));
console.log();

// 3. Introspection
console.log('3. Introspection:');
console.log('   Total keys:', cache.count());
console.log('   All keys:', cache.keys());
console.log('   Memory size:', cache.size(), 'bytes (approximate)');
console.log();

// 4. Add only when missing
console.log('4. Add only when missing:');
const added1 = cache.add('user:1', { id: 99, name: 'Override' });
console.log('   Try to add existing user:1:', added1, '(already exists)');
const added2 = cache.add('user:3', { id: 3, name: 'Charlie' });
...
const pulled = cache.pull('temp-token');
console.log('   Pulled temp-token:', pulled);
console.log('   Try to get again:', cache.get('temp-token', 'GONE'));
console.log();
...
cache.flush();
console.log('   After flush, count:', cache.count());
console.log();
