...
(async () => {
  ...
  await cache.put('app.name', 'Newton Cache Demo');
  await cache.put('app.version', '1.0.0');
  await cache.put('app.debug', true);
  await cache.put('api.endpoint', 'https://api.example.com');
  await cache.put('api.timeout', 5000);
  ...
  console.log('   app.name:', await cache.get('app.name'));
  console.log('   app.debug:', await cache.get('app.debug'));
  console.log('   api.timeout:', await cache.get('api.timeout'));
  console.log();
  ...
  console.log('   File size:', await cache.size(), 'bytes');
  console.log();
  ...
  console.log('   Data persisted! app.name:', await cache2.get('app.name'));
  console.log('   Session still exists:', await cache2.has('session:user123'));
  console.log();
