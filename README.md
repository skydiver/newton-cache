...
// local users can't read your cached values. Override if you need to:
const shared = new FileCache({ cachePath: "/var/tmp/my-cache", mode: 0o644 });
```
...
// The cache file defaults to mode 0o600 (owner read/write only). Override if needed:
const shared = new FlatFileCache({
  filePath: "/var/cache/my-app.json",
  mode: 0o644,
});
```
...
// Bound memory usage with LRU eviction. Once the cache holds `maxEntries`
// items, inserting a new key evicts the least-recently-used one. Reads and
// writes count as "use", so hot keys stay resident.
const bounded = new MemoryCache<string>({ maxEntries: 1000 });
```
...
**Stampede protection:** `remember`/`rememberForever` deduplicate concurrent misses. If several callers request the same missing key at once, the factory runs **once** and every caller awaits the same result. A factory rejection propagates to all waiters and clears the in-flight entry so the next call retries (failures are never cached). Dedup is per cache instance / per process — it fully protects `MemoryCache` and serializes work within a single process for the file adapters, but is not a cross-process lock.

```ts
// All three callers share a single fetchUsers() call:
const [a, b, c] = await Promise.all([
  cache.remember("users", 60, fetchUsers),
  cache.remember("users", 60, fetchUsers),
  cache.remember("users", 60, fetchUsers),
]);
```
...
Run `prune()` automatically on a background timer instead of calling it by hand:

```ts
// Prune expired entries every 60 seconds
cache.startAutoPrune(60);

// Stop the background timer (e.g. on shutdown)
cache.stopAutoPrune();
```

The timer is `unref`'d, so it never keeps the Node.js process alive on its own. `startAutoPrune` is idempotent — calling it again replaces the existing timer — and throws a `RangeError` if the interval is not a positive finite number. Background prune rejections are swallowed so a failing prune can't crash the host process.
...
- Optional `maxEntries` enables O(1) LRU eviction (Map insertion order; reads and writes bump recency)
